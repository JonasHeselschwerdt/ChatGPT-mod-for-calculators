...
// Needed as buffer for XBMs (equations etc.)
static uint8_t xbm_buf[BYTES_PER_XBM];
static char xbm_not_available[] = "XBM can not be rendered";
...
static char* changed_eq_render_server_address_msg[MAIN_DISPLAY_ROWS] = {
    STYLE_LINE,
    "  Changed equation  ",
    "  rendering server  ",
    STYLE_LINE
...
menu_entry_TypeDef eq_rendering_menu[];
// UI
...
void menu_cb_58(void);
void menu_cb_59(void);
void menu_cb_60(void);
void menu_cb_61(void);
void menu_cb_62(void);
...
static void chatview_show_equation(void);
...
static void UI_sidetask(void* arg){
    ...
    */
    ...
    while(1){
        ...
        if (UI.Sidedisplay_content == SIDEDISPLAY_SHOW_STATUS){
...
static void handle_pressed_keys_calc(Key_TypeDef* cur_pressed_keys){
    ...
    if (cur_pressed_keys[0].special_function == KEY_SHIFT_SPECIAL_FUNC){
        ...
        if (!strcmp(calculatormode.equation_page,UI.unlock_code)){
            ...
            // Turn on sidedisplay, show status screen
            device.side_display_on = 1;
            dep128064_power_ctrl(device.side_display_on);
            UI.Sidedisplay_content = SIDEDISPLAY_SHOW_STATUS;
            return;
        }
...
textinput_page_TypeDef enter_rend_serv =  {">Enter address      ",NO_SENSITIVE_INFO,  ai_main_menu,       NO_GOTO_PAGE,      textinput_cb_6};
...
static esp_err_t maybe_enter_chatview(void){
    ...
    // Always show Equation 0 or Error message if no equations exist
    chatviewmode.current_item_equation_index = 0;
    return ESP_OK;
}
...
static void chatview_show_equation(void){
    ...
    if (get_eq_rendering(UI.current_ai_model,
                        chatviewmode.current_chat_position,
                        chatviewmode.current_item_equation_index,xbm_buf) == ESP_OK){
        dep128064_show_XBM(xbm_buf,XBMS_WIDTH,XBMS_HEIGHT);
    }
    else{
        dep128064_show_info(xbm_not_available);
    }
}
...
static void handle_pressed_keys_chatview(Key_TypeDef* cur_pressed_keys){
    ...
    if (cur_pressed_keys[0].special_function == KEY_SHIFT_SPECIAL_FUNC){
        ...
        if ((chatviewmode.current_chat_position > 0) && (!chatviewmode.show_response)){
            ...
            chatviewmode.show_response = 1;
            chatview_load_and_format_response();
            // Show first equation if existent
            chatviewmode.current_item_equation_index = 0;
        }
        ...
        if (UI.Sidedisplay_content == SIDEDISPLAY_SHOW_STATUS){
            UI.Sidedisplay_content = SIDEDISPLAY_SHOW_EQUATION;
        }
        else{
            UI.Sidedisplay_content = SIDEDISPLAY_SHOW_STATUS;
            dep128064_clear_screen();
    ...
    else if (cur_pressed_keys[0].normal_meaning == 'r'){
        // Left bracket key (see Keypad LUT), naviate through equations
        if (get_eq_rendering(UI.current_ai_model,
                    chatviewmode.current_chat_position,
                    (chatviewmode.current_item_equation_index-1),xbm_buf) == ESP_OK){
            chatviewmode.current_item_equation_index--;
        }
    }
    else if (cur_pressed_keys[0].normal_meaning == 's'){
        // Right bracket key (see Keypad LUT), navigate through equations
        if (get_eq_rendering(UI.current_ai_model,
                    chatviewmode.current_chat_position,
                    (chatviewmode.current_item_equation_index+1),xbm_buf) == ESP_OK){
            chatviewmode.current_item_equation_index++;
    ...
    // Lastly, print screen(s)
    if (UI.Sidedisplay_content == SIDEDISPLAY_SHOW_EQUATION){
        chatview_show_equation();   // sidedisplay
    }
    chatview_print_chatitem_at_cursor();    // maindisplay
}   
...
{"AI Settings        ",    "Image handling     ",   img_del_after_prompt_menu,  main_menu,      NO_ENTER_CB,    NO_LEFT_CB,     NO_RIGHT_CB},
{"AI Settings        ",    "Equation rendering ",   eq_rendering_menu,          main_menu,      menu_cb_60,     NO_LEFT_CB,     NO_RIGHT_CB},
MENU_END
};

menu_entry_TypeDef filesys_main_menu[] = {
{"File system        ",    "Open File system   ",   NO_CHILD,                   main_menu,      menu_cb_15,     NO_LEFT_CB,     NO_RIGHT_CB},
{"File system        ",    "Get Files from host",   NO_CHILD,                   main_menu,      menu_cb_58,     NO_LEFT_CB,     NO_RIGHT_CB},
{"File system        ",    "Delete all files   ",   NO_CHILD,                   main_menu,      menu_cb_59,     NO_LEFT_CB,     NO_RIGHT_CB},
MENU_END
};
...
// Special case: Entry text Status changed dynamically
menu_entry_TypeDef eq_rendering_menu[] = {
{"Equation rendering ",    "Set server address ",   NO_CHILD,                   ai_main_menu,   menu_cb_61,     NO_LEFT_CB,     NO_RIGHT_CB},
{"Equation rendering ",    "Status:            ",   NO_CHILD,                   ai_main_menu,   menu_cb_62,     NO_LEFT_CB,     NO_RIGHT_CB},
MENU_END
};
...
void menu_cb_60(void){
    ...
    // Construct eq_rendering_menu
    if (get_eq_render_server_state()){
        eq_rendering_menu[1].entry_text[8] = 'O';
        eq_rendering_menu[1].entry_text[9] = 'N';
        eq_rendering_menu[1].entry_text[10] = ' ';
    }
    else{
        eq_rendering_menu[1].entry_text[8] = 'O';
        eq_rendering_menu[1].entry_text[9] = 'F';
        eq_rendering_menu[1].entry_text[10] = 'F';
    }
}
void menu_cb_61(void){

    // Enter textinputmode for setting equation rendering server
    enter_textinput_mode(&enter_rend_serv);
    UI.UI_mode = UI_MODE_TEXTINPUT;
    print_textinput_page_at_cursor();
}
void menu_cb_62(void){

    // Toggle server active state (equation rendering)
    set_equation_render_server(NULL,0,(get_eq_render_server_state()==0));
    if (get_eq_render_server_state()){
        eq_rendering_menu[1].entry_text[8] = 'O';
        eq_rendering_menu[1].entry_text[9] = 'N';
        eq_rendering_menu[1].entry_text[10] = ' ';
    ...
    // Render menu again since it has changed
    menu_mode_print_at_cursor_pos();
}
...
static void littleFS_init(void){
    ...
    // For chatdata of the AI conversations
