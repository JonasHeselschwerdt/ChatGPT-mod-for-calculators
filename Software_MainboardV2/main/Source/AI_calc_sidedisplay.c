...
// Info screen Info sign
static const uint8_t info_sign_bits[] = {0xf0,0x0f,0xf0,0x0f,0x3c,0x3c,0x3c,0x3c,0xff,0xff,0xff,0xff,0x3f,0xfc,0x3f,0xfc,0x3f,0xfc,0x3f,0xfc,0x3f,0xfc,0x3f,0xfc,0x3c,0x3c,0x3c,0x3c,0xf0,0x0f,0xf0,0x0f};
...
void dep128064_show_XBM(uint8_t* XBM_buf, uint8_t x_size, uint8_t y_size){
    ...
    // For printing XBMS (e.g. to render equations)
    if (device.side_display_on){
        u8g2_ClearBuffer(&u8g2);
        u8g2_SetBitmapMode(&u8g2, 1);
        u8g2_SetFontMode(&u8g2, 1);
        u8g2_DrawXBM(&u8g2, 0, ((SIDE_DISPLAY_PIX_Y - y_size)/2), x_size, y_size, XBM_buf);
        u8g2_SendBuffer(&u8g2);
    }
}
...
void dep128064_show_info(char* info_text){
    ...
    // Strlen of info_text can be up to 34, everything beyond that gets cut off
    if (device.side_display_on){
        u8g2_ClearBuffer(&u8g2);
        u8g2_SetBitmapMode(&u8g2, 1);
        u8g2_SetFontMode(&u8g2, 1);
        u8g2_DrawXBM(&u8g2, 4, 8, 16, 16, info_sign_bits);
        u8g2_SetFont(&u8g2, u8g2_font_6x12_tr);
        char text_buf[18];
        strncpy(text_buf,info_text,17);
        text_buf[17] = '\0';
        u8g2_DrawStr(&u8g2, 25, 13, text_buf);
        strncpy(text_buf,info_text+18,17);
        text_buf[17] = '\0';
        u8g2_DrawStr(&u8g2, 24, 25, text_buf);
        u8g2_SendBuffer(&u8g2);
    }
}
