...
// Sidedisplay-Content (for controlling access to side display)

typedef enum{
    SIDEDISPLAY_SHOW_STATUS,
    SIDEDISPLAY_SHOW_EQUATION
}Sidedisplay_content_TypeDef;
...
typedef struct{
    ...
    Sidedisplay_content_TypeDef Sidedisplay_content;    // not saved in NVS
}UI_TypeDef;
...
typedef struct{
    ...
    uint8_t current_item_equation_index;                    // only valid when showing response
